...
export default function AccountSecurity() {
  ...
  const [userIdForVerification, setUserIdForVerification] = useState<number | null>(null);
  ...
  const handleEmailChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && result.status === "success") {
        ...
        showToast("Kode verifikasi telah dikirim ke email baru Anda", "success");
        setPendingEmailChange(email);
        setUserIdForVerification(session?.backendUser?.id ? parseInt(session.backendUser.id) : null);
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (!passwordRegex.test(newPassword)) {
      showToast("Gunakan minimal 8 karakter dengan kombinasi huruf dan angka", "error");
      return;
    }
    ...
    try {
      ...
      showToast("Terjadi kesalahan saat mengubah password", "error");    } finally {
  ...
  return (
    <>
      <div className="w-full h-full flex flex-col items-center rounded-xl border border-[#CFCFCF] p-5 gap-2.5">
        <div className="flex flex-col items-start w-full h-full gap-10 overflow-y-auto">          <div className="flex flex-col items-start w-full gap-2.5">
              ...
              <div className="w-full flex flex-col items-start">
                ...
                {currentUserEmail && (
                  <p className="text-sm text-gray-600 mt-1">
                    Email saat ini: <span className="font-medium">{currentUserEmail}</span>
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="w-full p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm text-yellow-800">
                    Akun Anda terdaftar melalui Google. Email tidak dapat diubah.
                  </p>
                </div>
              </div>
            ) : (
              <form onSubmit={handleEmailChange} className="w-full flex flex-col gap-2.5">
                <Input
                  label="Email Baru"
                  type="email"
          ...
          </div>          <div className="flex flex-col items-start w-full gap-2.5">
            <div className="flex items-center justify-start w-full gap-3 pb-2.5 border-b border-[#CFCFCF]">
              <div className="flex items-center justify-center p-2.5 rounded-[30%] bg-gradient-to-br from-[#3BD5FF] to-[#367AF2]">
                <Icon
            ...
            {isGoogleUser ? (
              <div className="w-full p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <div className="flex items-center gap-2">
                  <Icon icon="mdi:information" className="text-yellow-600" />
                  <p className="text-sm text-yellow-800">
                    Akun Anda terdaftar melalui Google. Password tidak dapat diubah.
                  </p>
                </div>
              </div>
            ) : (
              <form onSubmit={handlePasswordChange} className="w-full flex flex-col gap-2.5">
                <Input
                  label="Password Lama"
                  type="password"
                  icon="mdi:lock"
